...
hg38otherN=`samtools view $path0/output/02.mapping/12.hg38other/sam/$j.hg38other.sam | wc -l`
...
###比较sam/bam文件差别###
#RNAN=`samtools view $path0/output/02.mapping/2.no_miRNA/rsem_bam/$j.miRNA.rsem.clean.bam | wc -l`
#echo $RNAN
#RNAM=`samtools view $path0/output/02.mapping/2.no_miRNA/sam/$j.miRNA.sam | wc -l`
#echo $RNAM
###发现文件没有差别###
...
k='hg38other';
echo hg38other;
samtools view $path0/output/02.mapping/$l.hg38other/bam/$j.hg38other.bam | awk 'BEGIN{FS=OFS="\t"}{print length($10)}' | sort -n | uniq -c | awk 'BEGIN{FS=" "; OFS="\t"}{print $2,$1}' | sort -nk1,1 | sed -e "s/^/$j\t$k\t/g" >> $path0/stat/$j.lengthN.stat.tsv
l=1;
